...
Nested rules live in `pipeline/CLAUDE.md` and `js/CLAUDE.md`. Keep long workflows out of this file. `.claude/` is gitignored, so `.claude/skills/` holds only local third-party skills.
...
Write every reply in ASD-STE100 Simplified Technical English.
...
3. If the fix is a workflow rather than a rule, ask where it goes. A skill in `.claude/skills/` cannot ride in the commit that step 4 asks for, because `.claude/` is gitignored.
...
- The reorg left stale `pipeline_utils` / `pipeline_fetch` / `pipeline_compute` names in comments across `pipeline/`, `cards/`, `js/` and `stuff_plus/`. They are navigation traps, not bugs. Fix one when you touch the line, and check any such reference resolves before trusting it.
...
- The `+` suffix is a contract: 100 is league average and one point is one percent. Process+ (named Hitter+ until 2026-09-16)/BB+/SD+/CT+ all honour it as of 2026-08-18. Never rescale a `+` stat to match another stat's spread: matching Process+ to wRC+'s full SD drops its slope to r and makes 115 worth 12%, not 15%. The deflation to `r x SD(wRC+)` IS the wRC+ scale.
- Process+'s run-truth `r` is measured LIVE each run so the slope stays exactly 1. A pool under 30 or an r outside 0.40-0.98 falls back to the frozen 0.82 and logs it. The value used ships in `metadata.processPlusStandardization.wrcScaleMatch.r`.
- Process+ (2026-09-16, replaced Hitter+) is the 52/17/31 composite of the UNSHRUNK, prior-free atoms (`_procBb` / `_procSd` / `_procCt`, private row keys in `pipeline/process_data.py`), NOT of the displayed BB+/SD+/CT+. Their n0 shrinkage and bat-tracking priors are predictive devices and pull small samples toward 100; `metadata.processPlusStandardization` (`bbRaw`/`sdRaw`/`ctRaw`) describes the unshrunk inputs. Never rebuild Process+ from the displayed columns; `pipeline/window_pool.py` and `scripts/tools/platoon_splits.py` carry the same construction.
- Percentile precision differs by family: the pipeline ranks FULL-precision values (`process_data.py` says so where the ranks are computed), while the Stuff+ family ranks ROUNDED ones (Stuff+ at 1 decimal, xRVOE/100 at 2, in `train_stuff.py`). Since 2026-09-24 `stuffScore` is STORED at 6 decimals (`STUFF_STORE_DECIMALS`, swapped in as the last step of `inject()`), but every Stuff+ rank still reads the 1-decimal value: `_pctl` in `train_stuff.py`, `_computePercentilesUnified` in `js/aggregator.js`, `_stuff_rank_key` in `cards/pitcher.py`, and hpERA/Pitcher+ read it before the swap. A new rank path that reads the stored value directly moves ties. Only in the second family can an affine rescale shift a few ranks by exactly 1 through tie-breaking; that is not a rank change. Check which family a stat is in before assuming either behaviour.
...
- A ROC/AAA row never resolves a combined row. `Utils.playerKey` groups on `mlbId`, so a ROC stint of a traded player shares a key with his MLB rows, and the combined row is built from MLB stints only. `compute_percentile_ranks_with_aaa` in `pipeline/compute.py` routes ROC rows (`_isROC`) out before the same check.
...
- PlateX/PlateZ are Savant-denominated since 2026-08-29. The gameday feed evaluates pZ at the front of the plate and Savant at the plate midpoint, so feed pZ reads higher by a pitch-dependent amount (see the plane line below; SzTop/SzBot and PlateX agree), which flipped borderline low pitches into the zone. `backfill_supplement.py` always-overwrites both columns from Savant on MLB AND ROC/AAA tabs (the minors feed has the same plane difference, measured 2026-08-30). The SCRAPE also merges them now, from the `gf` endpoint, which is fully populated one minute after a game goes Final while the Statcast Search CSV is still empty; it fails open, keeping the feed value and logging the count. `scripts/audits/resync_recent_platez.py` is RETIRED: it syncs toward the feed and would undo the backfill.
...
- A metric's prose definition has TWO homes: the module docstring and the `desc` string in `js/leaderboard.js`. The card footnote in `cards/pitcher.py` was a third until 2026-09-23, when the non-social card's footnote block was removed. The hpERA channel-set correction of 2026-08-27 reached `pipeline/eraplus.py` and the js tooltip but not the card, which then told every reader that hpERA contains hdERA's xwOBA term and that both are calibrated to future ERA. Both were false. After changing what a metric IS, grep its name across both homes, and do not add a card footnote that restates a definition.
...
- A recalibration of any Stuff+ input column owes a RETRAIN, not a score-only run. nVAA reads PlateZ (`_nvaa` in `train_stuff.py`), and the 2026-08-29 Savant recalibration moved every 2026 nVAA by ~0.09 deg (0.2 between-pitcher SD on FF/SI/FC/CH) under fold models trained on feed values; no retrain followed until at least 2026-09-02. Read retrain dates from `gh run list` durations (~50 min vs ~15): `stuff_metrics_history.csv` appends on every score-only run and cannot tell you.
- CI commits `data/ index.html trade.html catch.html css/ js/ stuff_plus/pitcher_stuff.csv` (the `git add` step in `.github/workflows/update-leaderboard.yml`). `pitcher_stuff.csv` is the low-support flag source that score-only runs read.
...
- Any 2026-side Loc+ verdict dated before 2026-08-29 ran on feed PlateZ, on average +0.086 ft = 0.054 znorm = 54% of `BIN_Z` above the Savant-denominated 2021-2025 caches (pitch-type dependent, see the plane line below). A/B arms shared the shift, so those verdicts stand; absolute vertical-geometry claims from that period do not. The per-type gates (`STABILIZE_N_PT` in `pipeline/locplus.py`) have since been re-measured on Savant PlateZ.
...
- The Stuff+ gate's records are `data/_gate_v2/results.json` (written by `stuff_gate_v2.py`), the `run_*.log` files and the agg pickles. `height_map()` reads `data/_gate_v2/pitcher_height.json`. Both JSON files are committed. `season_2026.pkl` is on Savant PlateZ, carries `l_loc`, and runs through 2026-09-04 (`build-loc` rebuilt every season frame on 2026-09-08). Every gate record since then scores against it, and the gate reuses any `agg_*` file that exists whatever the frame holds, so move the `agg_*` files aside before a rebuild that adds later dates, or the paired deltas mix two frames.
...
- A pitcher-level Loc+ objective without a STUFF control is gameable toward LESS location content: shipped Loc+ correlates -.11 with out-of-sample Stuff+ (-.22 with FF velo), a wider bandwidth erases that, and raw r rises to the grid edge (13 in / 0.75, z 5-6) on every target. The FF-velo partial removes only ~60%. Read the Stuff+-partialled columns of `scripts/research/locplus/locplus_target_audit.py` (within and next season, xRV and actual RV, paired bootstrap), which bracket an interior optimum. Measured 2026-09-17.
- Loc+ clamps a pitch outside its grid into the edge cell, and in two-strike counts an edge cell is a chase location: a changeup 34.6 in off the plate graded 149 (2026-09-19). From about 12 in outward the 6 in / 0.30 kernel also pulls the zone's swing and called-strike rates into the chase and waste cells, so the surface reads too flat there. Do not read a single-pitch or tiny-row Loc+ out there as a location verdict. A far-miss cutoff and an accurate outer region were both measured and not adopted (`scripts/research/locplus/locplus_farmiss_rule.py`, `locplus_geometry_calibration.py`): per-pitch accuracy and pitcher skill are different quantities here, so the edge behavior is a definition call, not a bug to fix.
- `scripts/ci/validate_output.py` hard-codes `DATA_DIR` to the repo. To validate a golden-harness output, load the module with `importlib`, set `DATA_DIR` to `runs/<NAME>/data`, and call `main()`.
...
- Expected movement (`pipeline/xmove.py`) is scored PER PITCH and the site and card only sum `sumXIVB`/`sumXHB` from the micro rows; never put model math back into `js/aggregator.js`. The shipped basis is the HITTER one (slot, extension, velocity): the spin + release-axis basis explains movement far better (R² 0.49 vs 0.27) but its residual explains RESULTS worse than raw movement, because it credits away the spin that is the weapon (`xmove_residual_value.py`, 5-season LOSO).
...
- The Stuff+ gate's `nxt_r` POOLS hands. A feature that shifts one hand's level (`is_lhp`: +0.040 pooled, z 4.9, 5/5) wins the gate while ranking no one better within a hand (-0.001). Decompose within group before reading any gate win as a ranking gain. Measured 2026-09-05. Since 2026-09-17 the gate summary prints the within-hand twin (`wh`, `wh_rend`, plus per-hand deltas) from the cached aggregates; read it beside `raw`. `nxt_r` also POOLS roles: relievers grade higher and post better next-season targets, so SHIPPED reads .406 pooled against .339 within role (5/5 pairs, 2026-09-23). Since then the summary also prints the within-role twin (`wr`, `wr_rend`, role = Y+1 starter share, cutoffs are conventions); read it too. Hand and role are guarded; no other group is.
...
- `data/_statcast{yr}_cache.pkl` columns are pandas NULLABLE dtypes (the 2025 cache at least). Convert with `pd.to_numeric(col, errors='coerce').to_numpy(dtype='float64', na_value=np.nan)` before any scalar test; `.values` hands back `pd.NA`, which raises `boolean value of NA is ambiguous` inside a plain `if` (2026-09-08, gate v2 `build-loc`, four seasons in before it hit).
- `maintenance.auto` stays `false` in this repo's `.git/config` (set 2026-09-09; a fresh clone needs it again). `auto-pull.sh` fetches every two minutes, and each fetch spawned a cruft repack of the 11 GB `.git` that did not wait for the previous one; eight stacked, holding 12.5 GB of RAM. Do not rewrite history to shrink `.git`: dropping the retired `data_embedded.js` measured 0.2 GB against a plain `git gc`, and the hashes cited in this file are the reason.
- The social card's pre-backfill state is a NON-BIP pitch without `RunExp` (the supplement writes it on 100% of pitches). A ball in play without `xwOBA` is NOT that state: Savant leaves ~1% of BIP unpriced for good and `compute_xrv` prices them at the league count mean. On 2026-09-10 the daily xRV estimate keyed on either hole and marked Lord's finished 9/9 card `xRV*` for one bunt. Key any 'not backfilled yet' test on `RunExp` coverage alone.
- A scratch-tab card fetches boxscores by the gamePks embedded in PitchID, and it must take them from the WINDOW rows (`pitches_by_pitcher`), never from `all_rows`. On 2026-09-13 the 09-12 daily card summed every game on the NEW tab and printed the season line (17.2 IP, 34 K) as the outing.
- When `auto-pull.sh` fires mid-session over a local `process_data` build, the autostash pop leaves the `data/` artifacts and the stamped pages UU and STAGES every other worktree change, including unrelated in-progress edits (2026-09-14: 1,343 staged, 47 unmerged). Resolve with `git checkout HEAD -- <artifact paths>` (a bare `git checkout --` refuses an unmerged path), then `git reset -- <source paths>` to unstage; the kept `autostash` entries in `git stash list` are the leftovers. Never commit from that state.
- `.githooks/pre-commit` refuses a commit whose staged `js/` file does not parse (`node --check` on the index content; without node it warns). Run `node --check js/*.js` by hand on a machine without the hook. The `desc` tooltip strings in `js/leaderboard.js` are single-quoted, so a bare apostrophe in prose ("the hitter side's 57", 2026-09-14) breaks the whole script at load and every leaderboard renders EMPTY, while the validator stays green (it does not parse JS) and a player-page check passes (player-page.js is a separate file). The site was blank from 19:40 to about 21:10 UTC that day. A leaderboard check must read a LEADERBOARD row, not only a player page.
- `data/_statcast{yr}_cache.pkl` carries `estimated_woba_using_speedangle` on strikeouts (0), walks (.69) and HBP rows, not only on batted balls. A ball-in-play filter on that column being non-null is wrong: it scales every per-player gap by his K and BB rates and reads the league wOBAcon-minus-xwOBAcon gap as -.068 instead of about 0. Filter balls in play on `bb_type.notna()` (the repo scripts already AND with a bip flag; ad hoc code on 2026-09-15 did not, and every career-gap number that morning had to be redone).
- Sheet sign conventions differ between the two hand-signed columns: `RelPosX` is NEGATIVE for a RHP (arm side = negative, medians -1.6 R / +1.8 L), `HorzBrk` is POSITIVE for a RHP sinker (arm side = positive, +15.5 R / -15.6 L). One `Throws` sign cannot serve both; the daily scan (`scripts/tools/daily_pitcher_scan.py`) flips them separately. Measured on the 2026 cache, 2026-09-16.
- The daily card's pre-backfill RunExp estimate (`_xrv_fill_estimates` in `cards/pitcher.py`) must fill BALLS IN PLAY too, keyed (count, event, outs, batted-ball type) with an event-mean fallback; `Runners` is a supplement column and cannot enter the key. Until 2026-09-21 it filled takes, whiffs and fouls only, so RV on a card built before the morning backfill omitted every hit and out on contact while xRV priced them: Herz 2026-09-21 printed 4 ER at RV +0.4 and a changeup at RV 0.0 against xRV -1.4. A pre-backfill RV whose sign disagrees with the line score is that gap. The Pitching+ tile's xRV slot takes the same fill since 2026-09-21; the tile, the table and the single-game window row (RV, xRV/100, Pitcher+) must read one xRV.
- `data/all_pitches_rs_cache.pkl` is gitignored and moves only when a LOCAL run rewrites it; CI's rebuild never reaches this machine. Before any work on recent games, run `python3 -m pipeline.refresh_pickle` (Sheets read, about 10 minutes on 2026-09-23) and read the `Date range` line it prints. Never report a game as missing from a stale pickle: on 2026-09-23 the cache ended 09-20 while Sheets held 09-22.
- The local `stuff_plus/stuff_models.pkl` is a hand download and goes stale at EVERY CI retrain, because a retrain on more data keeps the version string and the version guard passes. From 2026-09-11 to 09-23 the 09-02 bundle scored every local card about 2.5 points per pitch off the site (15.5% exact against the Sheets column; 99.5% after the refresh). `check_bundle_fresh` in `train_stuff.py` compares the loaded bundle with `data/stuff_bundle_info.json` (CI writes it every run) and the card stops with the refresh command. Never create that JSON locally: CI commits it, and an untracked local copy blocks auto-pull.
- `build_df`'s pass-1 fastball reference and its pass-2 pitch features must read the SAME movement columns. The 2026-07-02 switch to density-adjusted `xIndVrtBrk`/`xHorzBrk` reached the pitch side only, so `ivb_diff`/`hb_diff` subtracted a raw fastball from an adjusted pitch until v17 (2026-09-24): +0.3 in IVB on average, up to 3.4 in for Coors arms. `stuff_gate_v2.prepare()` rebuilds the diffs itself from the adjusted columns, so the gate scored a feature production did not compute. When a Stuff+ input changes, diff `build_df` against `prepare()` before trusting a gate verdict for production.
- Display a whole-number + stat with HALF-UP rounding on the exact value (per Wally 2026-09-24): `half_up` in `cards/pitcher.py`, `Math.round` on the site, `floor(x + 0.5)` in R. Python's `round()` and R's `round()` are half-to-even (116.5 printed 116 on cards and in `Daily.R` while the site showed 117), and rounding a 1-decimal copy double-rounds (the browser's `toFixed(1)` showed 170 of 3,831 pitch rows one point high). The per-pitch atoms stay `int(round())`: they must equal the Sheets integers.
