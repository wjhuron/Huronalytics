...
- The Stuff+ gate's records are `data/_gate_v2/results.json` (written by `stuff_gate_v2.py`), the `run_*.log` files and the agg pickles. `height_map()` reads `data/_gate_v2/pitcher_height.json`. Both JSON files are committed. Rebuild `season_2026.pkl` (frozen 2026-08-22, feed PlateZ) before any re-gate.
