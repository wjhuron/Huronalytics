"""locplus_stabilize_pt.py — measure regression constants (n_prior) for the
OVERALL Loc+ and for each pitch-type GROUP, using the final locked config and
grouping. The N where split-half r=0.5 is the Bayesian regression constant.

Final grouping: FF | SI | FC | SL(+ST,SW,SV) | CU(+KC,CS) | CH(+FS) | OTHER.
"""
...
print("building V3 (final grouping) ...",flush=True)
...
def crossing(by_p, Ns):
    for v in by_p.values(): random.shuffle(v)
    prev=None; cross=None; rows=[]
    for N in Ns:
        xs=[];ys=[]
        for vals in by_p.values():
            if len(vals)>=2*N: xs.append(sum(vals[:N])/N); ys.append(sum(vals[N:2*N])/N)
        r=pearson(xs,ys)
        if r is None: continue
        rows.append((N,len(xs),r))
...
rows,cross=crossing(bp,[50,75,100,150,200,300,400])
print("\nOVERALL  N : r")
print("  "+"  ".join(f"{N}:{r:.2f}" for N,_,r in rows))
print(f"  => n_prior(overall) ≈ {round(cross) if cross else '?'}")
...
for G in ['FF','SI','FC','SL','CU','CH']:
    ...
    rows,cross=crossing(bpg,[25,50,75,100,150,200,300])
    rstr="  ".join(f"{N}:{r:.2f}" for N,_,r in rows)
    print(f"  {G:3s}  {rstr}   => n_prior≈{round(cross) if cross else '>range'}")
