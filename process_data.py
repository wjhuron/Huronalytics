...
PITCH_PCTL_KEYS = list(METRIC_KEYS.values()) + ['spinEff'] + PITCH_STAT_KEYS
...
def compute_air_density(elevation_ft, temp_f):
    """Compute air density (kg/m³) from elevation (ft) and temperature (°F).
    Uses barometric formula assuming dry air (humidity adds ~1-2% error max)."""
    elevation_m = elevation_ft * 0.3048
    P = 101325 * (1 - 2.2558e-5 * elevation_m) ** 5.2556  # pressure in Pa
    T = (temp_f - 32) * 5 / 9 + 273.15  # temperature in Kelvin
    return P / (287.05 * T)


def fetch_game_air_density(game_pks):
    """Fetch air density for each game_pk from the MLB live feed.
    Returns dict of {game_pk: rho} where rho is in kg/m³."""
    game_rho = {}
    total = len(game_pks)
...
def main():
    ...
    # Read all pitches from AL and NL spreadsheets
    all_pitches = []
    total_sheets = 0
    for league, sheet_id in SPREADSHEET_IDS.items():
        sh = gc.open_by_key(sheet_id)
        print(f"[{league}] {sh.title} ({len(sh.worksheets())} sheets)")
        for i, ws in enumerate(sh.worksheets()):
            tab_name = ws.title.strip()
            if tab_name not in MLB_TEAMS:
                print(f"  Skipping {tab_name} (not MLB)")
                continue
    ...
    print(f"Read {len(all_pitches)} pitches from {total_sheets} team tabs")
    ...
    print(f"  Air density range: {min(rho_vals):.4f} - {max(rho_vals):.4f} kg/m³ "
          f"(std={STANDARD_RHO})")
    ...
    for (pitcher, team, pitch_type, throws), pitches in pitch_groups.items():
        ...
        # M = 0.96 * 2.58 * [1 - exp(-0.0857 * ε * R)]
        # Compute per-pitch M and R (with game-specific air density), average, then solve for ε
        M_vals = []
        R_vals = []
        for p in pitches:
            ...
            if D <= 0:
                continue
            # Get game-specific air density
            pid = p.get('PitchID', '')
            parts = str(pid).split('_')
            rho = game_rho.get(parts[0], STANDARD_RHO) if len(parts) == 3 else STANDARD_RHO
            # Normalized movement (inches → feet, adjust for distance and air density)
    ...
    # --- Spin Efficiency diagnostics ---
    se_values = [r for r in pitch_leaderboard if r.get('spinEff') is not None]
    se_null = len(pitch_leaderboard) - len(se_values)
    if se_values:
        se_by_pt = defaultdict(list)
        for r in se_values:
            se_by_pt[r['pitchType']].append(r['spinEff'])
        print(f"Spin Efficiency: {len(se_values)} rows computed, {se_null} null")
        for pt in sorted(se_by_pt):
            vals = se_by_pt[pt]
            print(f"  {pt}: avg={sum(vals)/len(vals):.1f}%, "
    ...
    # Group by (Batter, BTeam)
    # Only include hitters with an MLB team
