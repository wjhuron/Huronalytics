...
# Normalizes IVB/HB to league-average air density, removing elevation and temperature bias.
# Reference: pitch-count-weighted average across 2026 MLB games ≈ sea level, ~71°F, 50% RH.

WEATHER_CACHE_PATH = os.path.join(DATA_DIR, 'weather_cache.json')
REFERENCE_AIR_DENSITY = 1.191  # kg/m³ — league-average game conditions
...
def _compute_air_density(elevation_ft, temp_f, humidity_pct=50):
    """Calculate air density (kg/m³) from elevation, temperature, and humidity."""
    elevation_m = elevation_ft * 0.3048
    temp_c = (temp_f - 32) * 5 / 9
    temp_k = temp_c + 273.15
    P0 = 101325; L = 0.0065; T0 = 288.15; g = 9.80665
    M_air = 0.0289644; R_gas = 8.31447
    P = P0 * (1 - L * elevation_m / T0) ** (g * M_air / (R_gas * L))
    e_sat = 611.21 * math.exp((18.678 - temp_c / 234.5) * (temp_c / (257.14 + temp_c)))
    e = (humidity_pct / 100) * e_sat
    return ((P - e) * M_air + e * 0.018016) / (R_gas * temp_k)


def _estimate_humidity(condition):
    """Estimate relative humidity from weather condition string."""
...
def fetch_game_weather(game_dates):
    """Fetch weather/venue data for all MLB games on the given dates.
    Returns dict keyed by (date, team_abbrev) -> density_ratio (local / reference).
    Uses a persistent cache to avoid re-fetching."""
    cache = _load_weather_cache()
    # Determine which dates need fetching
    dates_to_fetch = [d for d in game_dates if d not in cache]
    if dates_to_fetch:
        print(f"  Fetching weather for {len(dates_to_fetch)} new date(s)")
    for date_str in dates_to_fetch:
        ...
        for dd in sched.get('dates', []):
            for g in dd.get('games', []):
                ...
                pk = g['gamePk']
                try:
                    gurl = f'https://statsapi.mlb.com/api/v1.1/game/{pk}/feed/live'
                    req2 = urllib.request.Request(gurl, headers={'User-Agent': 'Mozilla/5.0'})
                    with urllib.request.urlopen(req2, timeout=15) as resp2:
                        gdata = json.loads(resp2.read())
                except Exception as e:
                    print(f"    Error fetching game {pk}: {e}")
                    continue
                gd = gdata.get('gameData', {})
                ...
                rh = _estimate_humidity(weather.get('condition'))
                rho = _compute_air_density(elevation, temp_f, rh)
                ratio = rho / REFERENCE_AIR_DENSITY
                home_abbrev = TEAM_NAME_TO_ABBREV.get(
                    teams.get('home', {}).get('name', ''), None)
                away_abbrev = TEAM_NAME_TO_ABBREV.get(
                    teams.get('away', {}).get('name', ''), None)
                entry = {'ratio': round(ratio, 6), 'elevation': elevation,
                         'temp': temp_f, 'rho': round(rho, 4),
                         'home': home_abbrev, 'away': away_abbrev,
                         'venue': venue.get('name', ''),
                         'condition': weather.get('condition', ''),
                         'wind': weather.get('wind', '')}
                if home_abbrev:
                    date_games[home_abbrev] = entry
                if away_abbrev:
                    date_games[away_abbrev] = entry
                time_module.sleep(0.1)
        cache[date_str] = date_games
    _save_weather_cache(cache)

    # Build flat lookup: (date, team) -> density_ratio
    density_lookup = {}
    for date_str, games in cache.items():
        for team, entry in games.items():
            if isinstance(entry, dict) and 'ratio' in entry:
                density_lookup[(date_str, team)] = entry['ratio']
    return density_lookup


def apply_air_density_correction(all_pitches, label):
    """Correct IVB and HB on each pitch for local air density.
    Divides raw movement by (local_density / reference_density) to normalize
    all pitches to league-average atmospheric conditions.
    Modifies pitch dicts in-place. Skips MiLB pitches (no weather data)."""
    # Collect MLB game dates
    game_dates = sorted(set(
        normalize_date(p.get('Game Date')) for p in all_pitches
        if normalize_date(p.get('Game Date')) and p.get('_source', 'MLB') == 'MLB'))
    if not game_dates:
        return

    print(f"\n--- Air density correction ({label}) ---")
    density_lookup = fetch_game_weather(game_dates)
    if not density_lookup:
        print("  No weather data available, skipping correction")
        return
    ...
    for p in all_pitches:
        ...
        if not date or not team:
            continue
        ratio = density_lookup.get((date, team))
        if ratio is None:
            skipped += 1
            continue
        # Correct IVB and HB: divide by density ratio to normalize to reference
        ivb = safe_float(p.get('IndVertBrk'))
        hb = safe_float(p.get('HorzBrk'))
        if ivb is not None:
            p['IndVertBrk'] = str(round(ivb / ratio, 2))
        if hb is not None:
            p['HorzBrk'] = str(round(hb / ratio, 2))
        corrected += 1
        ratios_applied.append(ratio)

    if ratios_applied:
        avg_ratio = sum(ratios_applied) / len(ratios_applied)
        min_ratio = min(ratios_applied)
        max_ratio = max(ratios_applied)
        print(f"  Corrected {corrected} pitches (skipped {skipped})")
        print(f"  Density ratios: avg={avg_ratio:.4f}, min={min_ratio:.4f}, max={max_ratio:.4f}")
    else:
        print(f"  No pitches corrected (skipped {skipped})")
...
def process_game_type(all_pitches, label, mlb_id_cache, mlb_id_cache_path):
    ...
    # --- Apply air density correction to IVB/HB before any downstream processing ---
    apply_air_density_correction(all_pitches, label)
