const COLUMNS = {
  ...
  hitterStats: [
    ...
    { key: 'hWAR',        label: 'hWAR',     format: Utils.formatDecimal(1), sortType: 'numeric', desc: 'Huronalytics WAR for position players: deserved wins above replacement. Deserved batting runs (xwOBA on the pulled-air hitter basis, shrunk at 77 PA, park at .35 of the published factor; a bunt put in play is priced at its actual outcome, because no expected model describes a bunt), Savant baserunning run value (a runner Savant does not list is filled from his sprint speed and times on base, scaled so the league sums to zero) plus double-play runs from the 24-state run table, Savant fielding run value (range, arm, framing, blocking, throwing), a positional adjustment by innings that keeps the fWAR order with its spread compressed to .57 (measured on 2021 to 2026 bench bats by position; the same-fielder design reads flatter still, so this is the conservative end), and replacement at the 57 percent share of the 1000-WAR pool. Runs per win is the season constant pitcher hWAR uses. Season counting stat: it does not react to filters, and team rows sum it. fWAR, beside it, prices what happened; the gap between the two is outcome luck on contact plus the skills xwOBA does not see, and a hitter who beats his xwOBA every year reads higher there on purpose.', group: 'advanced' },
    ...
    { key: 'processPlus', label: 'Process+', format: Utils.formatInt, sortType: 'numeric', desc: 'Process+: a descriptive grade of the process behind the production. Weighted blend of BB+ (contact quality), SD+ (swing decisions) and CT+ (contact rate), built from the unshrunk, prior-free values so a small sample reads as what it was rather than as league average. 100 = league avg and one point is one percent, on the wRC+ ruler: the spread is deflated to r x SD(wRC+) with r measured live each run, which is what makes the slope exactly 1 (at the FULL wRC+ spread a 115 would be worth only ~12%). A gap vs wRC+ reads as results ahead of or behind process: luck, speed, spray and park live in the gap.', group: 'expected' },
  ...
  hitterSwingDecisions: [
    ...
    { key: 'ctPlus',      label: 'CT+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Contact+: leverage-weighted contact rate on swings. Frequency only — contact quality lives in BB+. 100 = league avg and one point is one percent: 115 means he made 15% more leverage-weighted contact than the league would on the same swings. Measured 2021-2026: frequency alone carries roughly ZERO wRC+ (quantity trades against quality), so a high CT+ is not by itself a production claim; its payoff arrives via the Process+ weights.', group: 'discipline' },
      ...
      if (!col.noPercentile && !isAvgRow) {
        ...
        if (pctl !== null && pctl !== undefined) {
          ...
          const HITTER_PA_STATS = {
            ...
            xwOBAsp: true, sprayVal: true, airPullPct: true, bbPlus: true, processPlus: true,
